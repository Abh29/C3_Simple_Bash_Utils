...
void    read_args(t_grep *arg, int argc, char **argv){
    ...
    while (i < argc)
    {
        ...
        if (*parm && (ft_strncmp(parm, "-", ft_strlen(parm)) == 0 || ft_strncmp(parm, "--", ft_strlen(parm)) == 0))
        ...
        else if (parm[0] == '-' && parm[1] == '-')
        {
            if (ft_strncmp(parm, "--file=", ft_strlen("--file=")) == 0) {
                ...
                i++;
                if (i == argc)
                    exit_msg("grep: option '--file' requires an argument\n", 2, 1);
                ...
                i++;
                if (i == argc)
                    exit_msg("grep: option '--regexp' requires an argument\n", 2, 1);
    ...
    if (!arg->patterns && !arg->patterns_path && !arg->patterns_list && !check_flag(arg, F_EMPTLINE))
...
void    read_argss(t_grep *arg, int argc, char **argv){
    ...
    while (i < argc) {
        if (ft_strncmp(argv[i], "-", ft_strlen(argv[i])) == 0 || ft_strncmp(argv[i], "-", ft_strlen(argv[i])) == 0)
            ft_lstadd_back(&(arg->files), ft_lstnew(ft_strdup(argv[i++])));
        i++;
    }
